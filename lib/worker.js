...
Worker.prototype.runServer = function (config) {
    ...
    // handle client-side certificate exchange
    var secureConnectionHandler = function (cleartextStream) {
    ...
    config.https.bind.forEach(function (options) {
        ...
        // test SNI with client SSL
        httpsServer.addContext('noask.substrait.com', {
            key: options.key,
            cert: options.cert,
            ca: null
        });
        ...
        httpsServer.on('secureConnection', secureConnectionHandler);
