...
var fs = require('fs'),
    ...
    util = require('util'),
    http = require('http'),
    https = require(process.env.NG_HTTPS ? './node/https2' : 'https'),
...
Worker.prototype.runServer = function (config) {
    ...
    (function () {
        ...
        // This is experimental SNI support
        // Will only work if NG_HTTPS env variable is passed, otherwise, will silently do nothing
        var SNICallback = function (serverNameIndication, callback) {
            log('Looking up certificate for ' + serverNameIndication);
            fs.readFile('/ssl/' + serverNameIndication + '.key', function (err, key) {
                if (err) {
                    log('No cert!');
                    callback();
                    return;
                }
                fs.readFile('/ssl/' + serverNameIndication + '.cert', function (err, cert) {
                    if (err) {
                        callback();
                        return;
                    }
                    log('Got a cert, keep on going!');
                    callback(crypto.createCredentials({key: key, cert: cert}).context);
                });
            });
        };
        ...
        options.SNICallback = process.env.NG_HTTPS ? SNICallback : null;
